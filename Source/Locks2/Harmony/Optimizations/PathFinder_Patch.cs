...
{
    [HarmonyPatch(typeof(PathFinder), nameof(PathFinder.FindPathNow), typeof(IntVec3), typeof(LocalTargetInfo),
        typeof(TraverseParms), typeof(PathFinderCostTuning?), typeof(PathEndMode), typeof(PathRequest.IPathGridCustomizer))]
    public class PathFinder_FindPath_Patch
    {
        ...
        private static int GetKey(TraverseParms traverseParms, LocalTargetInfo target)
        {
            ...
            unchecked
            {
                ...
                hash = hash ^ (target.Cell.GetHashCode() << 1);
            }
            return hash;
        }

        public static void Postfix(PawnPath __result, TraverseParms traverseParms, LocalTargetInfo target)
        {
            if (__result != PawnPath.NotFound) return;
            if (traverseParms.pawn == null) return;
            var key = GetKey(traverseParms, target);
